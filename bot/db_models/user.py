...
from sqlalchemy import Integer, BigInteger, String, select, DateTime, func
...
from models.enum_models import PaymentStatus
...
class User(Base):
    ...
    date_accession: Mapped[datetime.datetime] = mapped_column(DateTime, default=func.now())

    payments: Mapped[List["Payment"]] = relationship("Payment", back_populates="user", lazy="selectin")
    subscribe: Mapped["User"] = relationship("Subscribe", back_populates="user", uselist=False, lazy="selectin")

    @classmethod
    async def get_user_by_telegram_id(cls, telegram_id: int, session: AsyncSession) -> Self | None:
        """
    ...
    @classmethod
    async def get_not_successful_or_no_payments_telegram_ids(cls, session: AsyncSession) -> list[int]:
        """
        ...
        query = (
            ...
            .where(~cls.id.in_(select(subquery.c.user_id)))
        )
